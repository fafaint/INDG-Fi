...
def accuracy_from_loader(algorithm, loader, weights, debug=False):
    ...
    outs = np.array([])
    trgs = np.array([])
    for i, ((x, y, d), idx) in enumerate(loader):
        x = x.to(device)
        y = y.to(device)
        ...
        outs = np.append(outs, logits.argmax(1).cpu().numpy())
    ...
    acc = correct / total
    loss = losssum / total
    return acc, loss,f1_score(trgs, outs, average='macro')
...
class Evaluator:
    def __init__(
        self, test_envs, eval_meta, train_envs, logger, evalmode="fast", debug=False, target_env=None
    ):
    ...
    def evaluate(self, algorithm, ret_losses=False):
        ...
        for name, loader_kwargs, weights in self.eval_meta:
            ...
            acc, loss ,f1= accuracy(algorithm, loader_kwargs, weights, debug=self.debug)
            accuracies[name] = f1
            # accuracies[name+"f1"] = f1
            losses[name] = loss
